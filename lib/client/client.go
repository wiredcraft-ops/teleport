...
func (proxy *ProxyClient) ListResources(ctx context.Context, namespace string, resource proto.ResourceType, startKey string, limit int) ([]types.Resource, string, error) {
	authClient, err := proxy.CurrentClusterAccessPoint(ctx, false)
	if err != nil {
		return nil, "", trace.Wrap(err)
	}
