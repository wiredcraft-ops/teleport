...
func testResources(resourceType proto.ResourceType) ([]types.Resource, error) {
	var err error
	size := 50
...
func TestListResources(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"DatabaseServer": {
			resourceType:   proto.ResourceType_RESOURCE_TYPE_DATABASE_SERVER,
			resourceStruct: &types.DatabaseServerV3{},
		},
		"ApplicationServer": {
			resourceType:   proto.ResourceType_RESOURCE_TYPE_APPLICATION_SERVER,
...
func TestListAllResources(t *testing.T) {
	...
	for name, test := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			resources, err := clt.ListAllResources(ctx, defaults.Namespace, test.resourceType)
			require.NoError(t, err)
